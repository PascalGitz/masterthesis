...
Das vorangegangene Kapitel hat die Querschnittmodellbildung aussenvor gelassen. Das Ziel dieses Kapitels ist es, die Querschnittmodellbildung für Balken aufzuzeigen. Dazu sind die essentiellen Grössen der Balkentheorie beschrieben. Basierend auf diesen Grössen folgt der Abschnitt der Querschnittmodellbildung

## Bernoulli Balkentheorie

Der bereits mehrfach verwendete Begriff des Balkens bezieht sich in der gesamten Thesis auf die Balkentheorie nach Bernoulli. Dieser Abschnitt reisst die Balkentheorie in groben Zügen an. Es besteht lediglich der Anspruch, die später verwendeten Begriffe zu visualisieren. Grundlagen zur Balkentheorie können in @spura_einfuhrung_2019 (passende Quelle finden) nachgelesen werden.

Die Theorie nach Bernoulli geht von einem schubstarren Balken aus. Dabei gelten die folgenden Annahmen:
...
Unter der Berücksichtigung des Hookschen Gesetzes lassen sich die Normalspannungen ermitteln. Dargestellt in der @fig-allg_balken_normal.

![Normalspannungen am Balken](../imgs/allg_balken_normal.svg){#fig-allg_balken_normal}

Durch die Integration über die Fläche resultieren die beiden Biegemomente  und die Normalkraft, dargestellt in der @fig-allg_balken_sg. Für das Gleichgewicht sind zudem Querkräfte und das Torsionsmoment erforderlich. Diese sind ebenfalls dargestellt.

![Schnittgrössen am Balken](../imgs/allg_balken_sg.svg){#fig-allg_balken_sg}

Aufgrund der Hypothese des schubstarren Balkens lassen sich die Schubspannungen nicht anhand der Verzerrung bestimmen. Vielmehr sind die für das Gleichgewicht geforderten Querkräfte und Torsionsmomente anhand der Biegemomente zu bestimmen und daraus sind die Schubspannungen rückzurechnen. Der Bezug zwischen den Schubspannungen und den Schnittgrössen erfolgt ebenfalls durch die Integration über die Fläche. Dargestellet sind die Schubspannungen in der @fig-allg_balken_schub 
...
### Transformation vom Querschnitt zum Gelenk
...
Das Ziel ist es die Momentenfunktion gemäss der @def-gelenkfunktionen_raum zu ermitteln. Dazu ist eine Momenten-Krümmungs-Beziehung am Querschnitt zu definieren. Unter der Berücksichtigung der @eq-phi_to_chi folgt die Momenten-Verdrehungs-Beziehung. 
